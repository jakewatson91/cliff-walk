...
def q_learning(env, n_episodes, gamma=1.0, alpha=0.5, epsilon=0.1):
    ...
    for i in range(n_episodes):
        ...
        while True:
            ...
            # print(f"Q {state} actions: ", Q[next_state])
            next_action = np.max(Q[next_state])
            # print("Max: ", next_action)
            Q[state][action] = Q[state][action] + alpha * (reward + gamma * next_action - Q[state][action])

            score += reward
            if done == True:
                j = i + 1
                print(f"Score for episode {j}: {score}")
                break 
            state = next_state
    # print(Q)
    return Q
